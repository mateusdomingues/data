...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hello ' .. creatureGetName(cid) .. '! I\'m selling the first addon for 5k and the second addon for 10k. What do you wish?')
  		focus = cid
  		talk_start = os.clock()

  	elseif msgcontains(msg, 'hi') and (focus ~= cid) and getDistanceToCreature(cid) < 4 then
  		selfSay('Sorry, ' .. creatureGetName(cid) .. '. I\'m busy for now!')
		...
		elseif talk_state == 1 then
			if msgcontains(msg, 'yes') then
				if pay(cid,5000) then
					addon(cid, 1)
					selfSay('You got the first addons\' pack.')
					selfSay('/premium '.. creatureGetName(cid) ..', 0')
					...
					selfSay('Sorry, your cash amount isn\'t enough.')
				end
 			end
			talk_state = 0
		...
		elseif talk_state == 2 then
			if msgcontains(msg, 'yes') then
				if pay(cid,10000) then
					addon(cid, 2)
					selfSay('You got the second addons\' pack.')
					selfSay('/premium '.. creatureGetName(cid) ..', 0')
