...
function onUse(cid, item, frompos, item2, topos)
        ...
        if item.uid == 7009 and item.itemid == 1945 or item.itemid == 1946 then
			player1pos = {x=834, y=2208, z=7, stackpos=253} 
			player1 = getThingfromPos(player1pos)
 
			player2pos = {x=834, y=2207, z=7, stackpos=253} 
			player2 = getThingfromPos(player2pos)
 
			player3pos = {x=834, y=2206, z=7, stackpos=253} 
			player3 = getThingfromPos(player3pos)
 
			player4pos = {x=834, y=2205, z=7, stackpos=253} 
  ...
  if player1.itemid > 0 and player2.itemid > 0 and player3.itemid > 0 and player4.itemid > 0 then
		if getPlayerLevel(cid) >= config.MinLevel and getPlayerLevel(cid) <= config.MaxLevel then
			...
			if queststatus1 == -1 and queststatus2 == -1 and queststatus3 == -1 and queststatus4 == -1 then
				nplayer1pos = {x=764, y=2289, z=7} 
				nplayer2pos = {x=764, y=2290, z=7} 
				nplayer3pos = {x=764, y=2291, z=7} 
				nplayer4pos = {x=764, y=2292, z=7} 
	...
	doPlayerSendCancel(cid,"Voce precisa de quatro jogadores para esta quest.")
        ...
        elseif item.uid ==7009 and item.itemid == 1946 then
