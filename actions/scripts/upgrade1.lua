...
local fetishs = {
    [7800] = {chance = 100, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"},
    [7801] = {chance = 80, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"},
    [7802] = {chance = 60, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"},
    [7803] = {chance = 40, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"},
    [7804] = {chance = 20, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"},
    [7850] = {chance = 100, amount = 300, set = setItemDefense, get = getItemDefense, name = "Defense"}, -- only given when access >= 5
}
local minmax = {
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
    if item.actionid == 0 then
        ...
        doItemSetAttribute(item.uid, "name", "" ..fetishs[new].chance.."% fetish for "..fetishs[new].name.." +"..fetishs[new].amount)
    elseif fetishs[item.actionid] ~= nil then
        if fetishs[item.actionid].get(itemEx.uid) > 0 then
            if itemEx.actionid < 105 then
                ...
                doItemSetAttribute(itemEx.uid, getItemNameById(itemEx.itemid).." (Level "..(amount+1)..")")
