...
local stage = {Abilitar = {}, Desabilitar = {x},     --<< Abilitar Stages?? >>--            [{x};{}]
...
db.getResult(lvl_query);db.getResult(reset_query);db.getResult(nolook_query)
else
 doRemoveCreature(cid)
...
return true
   ...
   --=[install System]=--
      function installReset()
       if db.getResult("ALTER TABLE `players` ADD reset INT(11) NOT NULL DEFAULT 0;") then
