...
local function runQuery(query)
    ...
    if func then
        ...
        print("[EPIC System Error] Nenhuma funcao DB encontrada no TFS!")
        return false
    end
end
...
function onSay(cid, words, param) 
    if(words == "!buyepic") then 
        local itemId = 2137 -- ID da Some Golden Fruits
        local count = 1
        
        -- Remove o item da mochila do player
        if doPlayerRemoveItem(cid, itemId, count) then 
            local days = 30 -- Defina quantos dias a fruta dá
